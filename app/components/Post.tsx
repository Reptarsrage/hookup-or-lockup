...
function Post({
  ...
  isLoading,
  ...passThroughProps
}: PostProps & Omit<React.HtmlHTMLAttributes<HTMLDivElement>, "style">) {
  if (isLoading) {
    return (
      <animated.div {...passThroughProps}>
        <div
          role="status"
          className="absolute top-2/4 left-1/2 flex -translate-x-1/2  -translate-y-1/2 flex-col items-center justify-center gap-4"
        >
          <svg
            aria-hidden="true"
